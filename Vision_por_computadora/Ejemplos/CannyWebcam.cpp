...
int main() {
	...
	while (charCheckForEscKey != 27 && capWebcam.isOpened()) {            // until the Esc key is pressed or webcam connection is lost
		...
		cv::cvtColor(imgOriginal, imgGrayscale, 0);                   // convert to grayscale
		...
		cv::namedWindow("imgCanny", 1);          // or CV_WINDOW_AUTOSIZE for a fixed size window matching the resolution of the image
